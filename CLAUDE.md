...
`Model.history` is the single, chronologically-ordered list of everything above the live grid, owned by this package: rows a resize pushed out, plus rows output scrolled off — `Update` drains the emulator's own scrollback into it after every write, so the emulator's scrollback is always empty between writes (don't read scrollback from the emulator and expect content). The trailing `Model.pullable` rows of it are owed back: every `SetSize` re-wraps them together with the live rows and returns whatever fits to the live grid. The emulator/pty are never sized below 1x1 (`minEmuWidth`/`minEmuHeight`) — `Model.width/height` are what the caller asked for, `emuW/emuH` what the emulator is really at. See `docs/superpowers/specs/2026-09-20-terminal-history-pull-back-design.md`.
