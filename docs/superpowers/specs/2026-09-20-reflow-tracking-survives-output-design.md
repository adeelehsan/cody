...
Anything this can't see through — a scroll region, reverse index, insert/delete line, a full-scrollback cap hiding the shift — just makes rows stop matching, which degrades to the heuristic: what every row had before the record existed. A row rewritten with byte-identical content (the SIGWINCH prompt redraw itself) keeps its state, correctly.
